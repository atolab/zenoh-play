...
#[derive(Debug)]
struct OperatorMinusOne;

impl Node for OperatorMinusOne {
...
impl Operator for OperatorMinusOne {
    fn input_rule(
        &self,
    ...
    fn run(
        ...
        let mut val = number.value();
        if val > 0 {
            val -= 1;
        }

        results.insert(OUTPUT.into(), Data::from::<DUsize>(DUsize::new(val)));

        // Sleep.
        let mut rng = thread_rng();
        let sleep_time = rng.gen_range(70..=100);
        println!("[OperatorMinusOne] Sleeping for {}ms", sleep_time);
...
fn register() -> ZFResult<Arc<dyn Operator>> {
    Ok(Arc::new(OperatorMinusOne) as Arc<dyn Operator>)
}
