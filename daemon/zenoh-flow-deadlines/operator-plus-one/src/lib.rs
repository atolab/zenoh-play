...
#[derive(Debug)]
struct OperatorPlusOne;

impl Node for OperatorPlusOne {
...
impl Operator for OperatorPlusOne {
    fn input_rule(
        &self,
    ...
    fn run(
        ...
        results.insert(
            OUTPUT.into(),
            Data::from::<DUsize>(DUsize::new(number.value() + 1)),
        );

        // Sleep.
        let mut rng = thread_rng();
        let sleep_time = rng.gen_range(25..=75);
        println!("[OperatorPlusOne] Sleeping for {}ms", sleep_time);
...
fn register() -> ZFResult<Arc<dyn Operator>> {
    Ok(Arc::new(OperatorPlusOne) as Arc<dyn Operator>)
}
